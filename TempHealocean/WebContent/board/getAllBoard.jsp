...
%>
   <tr height="30">
	...
	<%}%>
      ...
      <a href="boardController?choice=get-board&num=<%=board.getNum()%>&pageNum=<%=pageNum%>">
           <%=board.getSubject()%></a> 
          <% if(board.getReadcount()>=20){%>
         <img src="board/images/hot.gif" border="0"  height="16"><%}%> </td>
...
<%
    if (count > 0) {
        ...
        if (startPage > 10) {    %>
        <a href="boardController?choice=getAll-board&pageNum=<%= startPage - 10 %>&startRow=<%=(startPage - 1) * 10 + 1%>&endRow=<%=startPage * 10%>">[이전]</a>
<%      }
        for (int i = startPage ; i <= endPage ; i++) {  %>
        <a href="boardController?choice=getAll-board&pageNum=<%= i %>&startRow=<%=(i - 1) * 10 + 1%>&endRow=<%=i * 10%>">[<%= i %>]</a>
<%                                      
        }
        if (endPage < pageCount) {  %>
        <a href="boardController?choice=getAll-board&pageNum=<%= startPage + 10 %>&startRow=<%=(startPage - 1) * 10 + 1%>&endRow=<%=startPage * 10%>">[다음]</a>
<%
        }
    }
%>
