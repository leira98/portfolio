...
public class BoardEntity {
	...
	public void insertBoard(BoardVO board) throws Exception {			
        ...
        try {
            ...
            // 쿼리를 작성
            sql = "insert into board(num,writer,email,subject,passwd,reg_date,readcount,";
		    sql+="ref,re_step,re_level,content) values(seq_log.nextval,?,?,?,?,?,?,?,?,?,?)";
   ...
   public ArrayList getBoardList(int start, int end) throws Exception {
       ...
       try {
           conn = ds.getConnection();           
           pstmt = conn.prepareStatement("select * from (select rownum numrow, A.* from(select * from board)A) where numrow >= ? and numrow <= ? order by ref desc, re_step asc");        		   
